...
const DocumentPage = ({params}) => {
  ...
  const [documentProvider, setDocumentProvider] = useState(userDetails.name);
  ...
  const handleSubmit = async () => {
    try {
     console.log(params.aadharNumber)
      const user = JSON.parse(localStorage.getItem('userInfo'));
      console.log(user);
      console.log(diseaseTypeIndex, [documentTypeIndex, documentProvider, cid])
      
      const res = await contractInstance.methods.addDiseaseDocsByDoctor(params.aadharNumber,diseaseTypeIndex, [documentTypeIndex, documentProvider, cid,1792402588]).send({from : user.address});
